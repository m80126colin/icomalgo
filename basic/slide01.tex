...
\begin{frame}[fragile]
  \frametitle{算術運算子}
  \input{./res/operator_arithmetic.tex}
  \begin{exampleblock}{註}<2->
    \begin{itemize}
...
\begin{frame}[fragile]
  ...
  \begin{exampleblock}{$1-2-3=?$}
    \begin{itemize}[<+->]
    \item 我們直觀上會先算 $1-2=-1$，再算 $\alert{-1}-3=-4$。
    \item 因此 C++ 在設計上也會把加減乘除餘的\alert{結合性}「設定」成\alert{從左到右算}。
    \end{itemize}
  \end{exampleblock}
  \begin{table}[h]
...
\begin{frame}
  ...
  \begin{exampleblock}{$1+2*3-4=?$}
    \begin{itemize}[<+->]
    \item 我們的運算規則：「\alert{先乘除餘，後加減}」。
    \item 因此 C++ 發展出一套規則：\alert{運算順序}
      \begin{itemize}[<+->]
      \item 運算順序小的優先運算
